...
namespace System.Reflection.Utils
{
	public class CodeInstruction
	{
		...
		// Internal parameterless constructor that AccessUtils.CreateInstance can use, ensuring that labels/blocks are initialized.
		...
		{
			var method = AccessUtils.Method(type, name, parameters, generics);
		...
		{
			var method = AccessUtils.Method(typeColonMethodname, parameters, generics);
		...
		public static CodeInstruction CallClosure<T>(T closure) where T : Delegate
		{
			...
			if (preserveContext)
			{
				...
				il.Emit(OpCodes.Ldsfld, AccessUtils.Field(typeof(Transpilers), nameof(State.closureCache)));
				il.Emit(OpCodes.Ldc_I4, n);
				il.Emit(OpCodes.Callvirt, AccessUtils.PropertyGetter(typeof(Dictionary<int, Delegate>), "Item"));
			}
			else
			{
				if (closure.Target == null)
					...
					il.Emit(OpCodes.Newobj, AccessUtils.FirstConstructor(targetType, x => x.IsStatic == false && x.GetParameters().Length == 0));

				il.Emit(OpCodes.Ldftn, closure.Method);
				il.Emit(OpCodes.Newobj, AccessUtils.Constructor(typeof(T), new[] { typeof(object), typeof(IntPtr) }));
			}

			for (var i = 0; i < parameters.Length; i++)
				il.Emit(OpCodes.Ldarg, i);

			il.Emit(OpCodes.Callvirt, AccessUtils.Method(typeof(T), nameof(Action.Invoke)));
			il.Emit(OpCodes.Ret);
		...
		{
			var field = AccessUtils.Field(type, name);
