...
namespace System.Reflection.Utils
{
	...
	public class PatchProcessor
	{
		...
		RuntimeMethod prefix;
		RuntimeMethod postfix;
		RuntimeMethod transpiler;
		RuntimeMethod finalizer;
		...
		///
		public PatchProcessor(Runtime instance, MethodBase original)
		{
		...
		/// <param name="prefix">The prefix as a <see cref="RuntimeMethod"/></param>
		/// <returns>A <see cref="PatchProcessor"/> for chaining calls</returns>
		///
		public PatchProcessor AddPrefix(RuntimeMethod prefix)
		{
			this.prefix = prefix;
			return this;
		}
		...
		{
			prefix = new RuntimeMethod(fixMethod);
			return this;
		}

		/// <summary>Adds a postfix</summary>
		/// <param name="postfix">The postfix as a <see cref="RuntimeMethod"/></param>
		/// <returns>A <see cref="PatchProcessor"/> for chaining calls</returns>
		///
		public PatchProcessor AddPostfix(RuntimeMethod postfix)
		{
		...
		{
			postfix = new RuntimeMethod(fixMethod);
			return this;
		}

		/// <summary>Adds a transpiler</summary>
		/// <param name="transpiler">The transpiler as a <see cref="RuntimeMethod"/></param>
		/// <returns>A <see cref="PatchProcessor"/> for chaining calls</returns>
		///
		public PatchProcessor AddTranspiler(RuntimeMethod transpiler)
		{
		...
		{
			transpiler = new RuntimeMethod(fixMethod);
			return this;
		}

		/// <summary>Adds a finalizer</summary>
		/// <param name="finalizer">The finalizer as a <see cref="RuntimeMethod"/></param>
		/// <returns>A <see cref="PatchProcessor"/> for chaining calls</returns>
		///
		public PatchProcessor AddFinalizer(RuntimeMethod finalizer)
		{
		...
		{
			finalizer = new RuntimeMethod(fixMethod);
			return this;
		}
		...
		public static IEnumerable<MethodBase> GetAllPatchedMethods()
		{
			lock (locker)
			{
				return RuntimeSharedState.GetPatchedMethods();
			}
		}
		...
		///
		public MethodInfo Patch(bool ignoreDynamicMethod = false)
		{
			if (original is null)
			...
			lock (locker)
			{
				var patchInfo = RuntimeSharedState.GetPatchInfo(original) ?? new PatchInfo();
				...
				RuntimeSharedState.UpdatePatchInfo(original, replacement, patchInfo);
				return replacement;
			}
		}

		/// <summary>Unpatches patches of a given type and/or Runtime ID</summary>
		/// <param name="type">The <see cref="RuntimePatchType"/> patch type</param>
		/// <param name="runtimeID">Runtime ID or <c>*</c> for any</param>
		/// <returns>A <see cref="PatchProcessor"/> for chaining calls</returns>
		///
		public PatchProcessor Unpatch(RuntimePatchType type, string runtimeID)
		{
			lock (locker)
			{
				var patchInfo = RuntimeSharedState.GetPatchInfo(original);
				if (patchInfo is null) patchInfo = new PatchInfo();

				if (type == RuntimePatchType.All || type == RuntimePatchType.Prefix)
					patchInfo.RemovePrefix(runtimeID);
				if (type == RuntimePatchType.All || type == RuntimePatchType.Postfix)
					patchInfo.RemovePostfix(runtimeID);
				if (type == RuntimePatchType.All || type == RuntimePatchType.Transpiler)
					patchInfo.RemoveTranspiler(runtimeID);
				if (type == RuntimePatchType.All || type == RuntimePatchType.Finalizer)
					patchInfo.RemoveFinalizer(runtimeID);
				var replacement = PatchFunctions.UpdateWrapper(original, patchInfo);
		...
		{
			PatchInfo patchInfo;
			lock (locker) { patchInfo = RuntimeSharedState.GetPatchInfo(method); }
		...
		{
			return PatchFunctions.GetSortedPatchMethods(original, patches);
		}
		...
		/// <returns>A dictionary containing assembly version keyed by Runtime ID</returns>
		///
		public static Dictionary<string, Version> VersionInfo(out Version currentVersion)
		{
			currentVersion = typeof(Runtime).Assembly.GetName().Version;
			var assemblies = new Dictionary<string, Assembly>();
			GetAllPatchedMethods().Do(method =>
			{
				PatchInfo info;
				lock (locker) { info = RuntimeSharedState.GetPatchInfo(method); }
			...
			assemblies.Do(info =>
			{
				var assemblyName = info.Value.GetReferencedAssemblies().FirstOrDefault(a => a.FullName.StartsWith("0Runtime, Version", StringComparison.Ordinal));
