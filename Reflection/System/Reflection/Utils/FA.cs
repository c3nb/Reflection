...
namespace System.Reflection.Utils
{
	...
	///
	[Obsolete("Use AccessUtils.FieldRefAccess<T, S> for fields and AccessTools.MethodDelegate<Func<T, S>> for property getters")]
	...
	/// <returns>An delegate</returns>
	///
	[Obsolete("Use AccessUtils.FieldRefAccess<T, S> for fields and AccessTools.MethodDelegate<Action<T, S>> for property setters")]
	...
	public static class FastAccess
	{
		...
		public static InstantiationHandler<T> CreateInstantiationHandler<T>()
		{
			...
			var dynamicMethod = new DynamicMethod($"InstantiateObject_{typeof(T).Name}", typeof(T), null, true);
		...
		///
		[Obsolete("Use AccessUtils.MethodDelegate<Func<T, S>>(PropertyInfo.GetGetMethod(true))")]
		...
		/// <returns>The new getter delegate</returns>
		///
		[Obsolete("Use AccessUtils.FieldRefAccess<T, S>(fieldInfo)")]
		...
		/// <returns>The new getter delegate</returns>
		///
		[Obsolete("Use AccessUtils.FieldRefAccess<T, S>(name) for fields and " +
			"AccessUtils.MethodDelegate<Func<T, S>>(AccessUtils.PropertyGetter(typeof(T), name)) for properties")]
		public static GetterHandler<T, S> CreateFieldGetter<T, S>(params string[] names)
		{
			foreach (var name in names)
			{
				var field = typeof(T).GetField(name, AccessUtils.all);
				if (field is object)
					return CreateGetterHandler<T, S>(field);

				var property = typeof(T).GetProperty(name, AccessUtils.all);
		...
		{
			return new DynamicMethod($"DynamicGet_{type.Name}", typeof(S), new Type[] { typeof(T) }, true);
		}

		static DynamicMethod CreateSetDynamicMethod<T, S>(Type type)
		{
			return new DynamicMethod($"DynamicSet_{type.Name}", typeof(void), new Type[] { typeof(T), typeof(S) }, true);
		}
	}
}
