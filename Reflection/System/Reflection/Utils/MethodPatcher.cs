...
namespace System.Reflection.Utils
{
	internal class MethodPatcher
	{
		...
		internal MethodPatcher(MethodBase original, MethodBase source, List<MethodInfo> prefixes, List<MethodInfo> postfixes, List<MethodInfo> transpilers, List<MethodInfo> finalizers)
		{
			if (original is null)
			...
			returnType = AccessUtils.GetReturnedType(original);
		...
		internal static DynamicMethodDefinition CreateDynamicMethod(MethodBase original, string suffix)
		{
			...
			{
				if (AccessUtils.IsStruct(original.DeclaringType))
					parameterTypes.Insert(0, original.DeclaringType.MakeByRefType());
				else
					parameterTypes.Insert(0, original.DeclaringType);
			}

			var returnType = useStructReturnBuffer ? typeof(void) : AccessUtils.GetReturnedType(original);
		...
		void EmitCallParameter(MethodInfo patch, Dictionary<string, LocalBuilder> variables, LocalBuilder runOriginalVariable, bool allowFirsParamPassthrough, out LocalBuilder tmpObjectVar, List<KeyValuePair<LocalBuilder, Type>> tmpBoxVars)
		{
			...
			foreach (var patchParam in parameters)
			{
				...
				if (patchParam.Name == INSTANCE_PARAM)
				{
					...
					{
						var instanceIsRef = original.DeclaringType is object && AccessUtils.IsStruct(original.DeclaringType);
				...
				if (patchParam.Name.StartsWith(INSTANCE_FIELD_PREFIX, StringComparison.Ordinal))
				{
					...
					if (fieldName.All(char.IsDigit))
					{
						...
						fieldInfo = AccessUtils.DeclaredField(original.DeclaringType, int.Parse(fieldName));
					...
					{
						fieldInfo = AccessUtils.Field(original.DeclaringType, fieldName);
						if (fieldInfo is null)
				...
				{
					var returnType = AccessUtils.GetReturnedType(original);
				...
				else
				{
					...
					if (idx == -1)
					{
						var runtimeMethod = RuntimeMethodExtensions.GetMergedFromType(patchParam.ParameterType);
						if (runtimeMethod.methodType is null) // MethodType default is Normal
							runtimeMethod.methodType = MethodType.Normal;
						var delegateOriginal = runtimeMethod.GetOriginalMethod();
						if (delegateOriginal is MethodInfo methodInfo)
						{
							...
							if (delegateConstructor is object)
							{
								...
								if (methodInfo.IsStatic is false && runtimeMethod.nonVirtualDelegate is false)
								{
		...
		static bool PrefixAffectsOriginal(MethodInfo fix)
		{
			...
			return fix.GetParameters().Any(p =>
			{
				...
				if (AccessUtils.IsValue(type) is false && AccessUtils.IsStruct(type) is false) return true;
			...
			prefixes
				.Do(fix =>
				{
					...
					if (tmpObjectVar != null)
					{
						...
						emitter.Emit(OpCodes.Unbox_Any, AccessUtils.GetReturnedType(original));
