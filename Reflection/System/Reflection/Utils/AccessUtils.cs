	...
	public static class AccessUtils
	{
		...
		public static MethodInfo Method(Type type, string name, Type[] parameters = null, Type[] generics = null)
		{
			...
			if (parameters is null)
			{
				...
				catch (AmbiguousMatchException ex)
				{
					...
					if (result is null)
					{
						throw new AmbiguousMatchException($"Ambiguous match in Runtime patch for {type}:{name}", ex);
					}
				}
			}
			else
		...
		static FieldRef<T, F> FieldRefAccessInternal<T, F>(FieldInfo fieldInfo, bool needCastclass)
		{
			...
			var dm = new DynamicMethod($"__refget_{delegateInstanceType.Name}_fi_{fieldInfo.Name}",
				typeof(F), new[] { delegateInstanceType }, true);
			typeof(DynamicMethod).GetField("m_returnType", all).SetValue(dm, typeof(F).MakeByRefType());
		...
		static StructFieldRef<T, F> StructFieldRefAccessInternal<T, F>(FieldInfo fieldInfo) where T : struct
		{
			...
			var dm = new DynamicMethod($"__refget_{typeof(T).Name}_struct_fi_{fieldInfo.Name}",
				typeof(F), new[] { typeof(T).MakeByRefType() }, true);
			typeof(DynamicMethod).GetField("m_returnType", all).SetValue(dm, typeof(F).MakeByRefType());
			var il = dm.GetILGenerator();
			il.Emit(OpCodes.Ldarg_0);
			il.Emit(OpCodes.Ldflda, fieldInfo);
			...
			return (StructFieldRef<T, F>)dm.CreateDelegate(typeof(StructFieldRef<T, F>));
		}
		...
		static FieldRef<F> StaticFieldRefAccessInternal<F>(FieldInfo fieldInfo)
		{
			...
			var dm = new DynamicMethod($"__refget_{fieldInfo.DeclaringType?.Name ?? "null"}_static_fi_{fieldInfo.Name}",
				typeof(F), new Type[0], true);
			typeof(DynamicMethod).GetField("m_returnType", all).SetValue(dm, typeof(F).MakeByRefType());
			var il = dm.GetILGenerator();
			il.Emit(OpCodes.Ldsflda, fieldInfo);
			il.Emit(OpCodes.Ret);
		...
		public static DelegateType MethodDelegate<DelegateType>(MethodInfo method, object instance = null, bool virtualCall = true) where DelegateType : Delegate
		{
			...
			if (instance is null)
			{
				...
				var dmd = new DynamicMethod(
					"OpenInstanceDelegate_" + method.Name,
					method.ReturnType,
					parameterTypes, declaringType, true);
			...
			if (IsMonoRuntime)
			{
				var dmd = new DynamicMethod(
					...
					delegateType,
					new[] { typeof(object) }, delegateType, true);
				var ilGen = dmd.GetILGenerator();
				ilGen.Emit(OpCodes.Ldarg_0);
				ilGen.Emit(OpCodes.Ldftn, method);
				ilGen.Emit(OpCodes.Newobj, delegateType.GetConstructor(new[] { typeof(object), typeof(IntPtr) }));
				ilGen.Emit(OpCodes.Ret);
				return (DelegateType)dmd.Invoke(null, new object[] { instance });
			}
			return (DelegateType)Activator.CreateInstance(delegateType, instance, method.MethodHandle.GetFunctionPointer());
		}

		/// <summary>Creates a delegate for a given delegate definition, attributed with [<see cref="System.Reflection.Utils.RuntimeDelegate"/>]</summary>
		/// <typeparam name="DelegateType">The delegate Type, attributed with [<see cref="System.Reflection.Utils.RuntimeDelegate"/>]</typeparam>
		/// <param name="instance">
		/// Only applies for instance methods. If <c>null</c> (default), returned delegate is an open (a.k.a. unbound) instance delegate
		/// where an instance is supplied as the first argument to the delegate invocation; else, delegate is a closed (a.k.a. bound)
		/// instance delegate where the delegate invocation always applies to the given <paramref name="instance"/>.
		/// </param>
		/// <returns>A delegate of given <typeparamref name="DelegateType"/> to the method specified via [<see cref="System.Reflection.Utils.RuntimeDelegate"/>]
		...
		/// determined from the [<see cref="System.Reflection.Utils.RuntimeDelegate"/>] attributes on <typeparamref name="DelegateType"/>,
		/// and the given <paramref name="instance"/> (for closed instance delegates).
		/// </remarks>
		///
		public static DelegateType RuntimeDelegate<DelegateType>(object instance = null) where DelegateType : Delegate
		{
			var runtimeMethod = RuntimeMethodExtensions.GetMergedFromType(typeof(DelegateType));
			if (runtimeMethod.methodType is null) // MethodType default is Normal
				runtimeMethod.methodType = MethodType.Normal;
			var method = runtimeMethod.GetOriginalMethod() as MethodInfo;
			if (method is null)
				throw new NullReferenceException($"Delegate {typeof(DelegateType)} has no defined original method");
			return MethodDelegate<DelegateType>(method, instance, runtimeMethod.nonVirtualDelegate is false);
		}
		...
		public static MethodBase GetOutsideCaller()
		{
			...
			foreach (var frame in trace.GetFrames())
			{
				...
				if (method.DeclaringType?.Namespace != "System.Reflection.Utils")
					return method;
			}
