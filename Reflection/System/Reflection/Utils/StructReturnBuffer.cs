...
#pragma warning restore CS0169
	...
	internal class StructReturnBuffer
	{
		...
		{
			var returnType = AccessUtils.GetReturnedType(method);
			if (AccessUtils.IsStruct(returnType) is false) return false;
			if (AccessUtils.IsMonoRuntime is false && method.IsStatic) return false;
		...
		static bool HasStructReturnBuffer()
		{
			if (AccessUtils.IsMonoRuntime)
			{
				lock (hasTestResult_Mono_lock)
				{
					if (hasTestResult_Mono is false)
					{
						...
						var original = AccessUtils.DeclaredMethod(typeof(Sandbox), nameof(Sandbox.GetStruct_Mono));
						var replacement = AccessUtils.DeclaredMethod(typeof(Sandbox), nameof(Sandbox.GetStructReplacement_Mono));
			...
			lock (hasTestResult_Net_lock)
			{
				if (hasTestResult_Net is false)
				{
					...
					var self = new StructReturnBuffer();
					var original = AccessUtils.DeclaredMethod(typeof(Sandbox), nameof(Sandbox.GetStruct_Net));
					var replacement = AccessUtils.DeclaredMethod(typeof(Sandbox), nameof(Sandbox.GetStructReplacement_Net));
					_ = Memory.DetourMethod(original, replacement);
