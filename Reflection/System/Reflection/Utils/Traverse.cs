...
namespace System.Reflection.Utils
{
	...
	public class Traverse
	{
		...
		/// <param name="name">The type name, for format see <see cref="AccessUtils.TypeByName"/></param>
		/// <returns>A <see cref="Traverse"/> instance</returns>
		///
		public static Traverse CreateWithType(string name)
		{
			return new Traverse(AccessUtils.TypeByName(name));
		}
		...
		public object GetValue()
		{
			...
			if (_info is PropertyInfo)
				return ((PropertyInfo)_info).GetValue(_root, AccessUtils.all, null, _params, CultureInfo.CurrentCulture);
		...
		{
			if (_info is FieldInfo)
				((FieldInfo)_info).SetValue(_root, value, AccessUtils.all, null, CultureInfo.CurrentCulture);
			if (_info is PropertyInfo)
				((PropertyInfo)_info).SetValue(_root, value, AccessUtils.all, null, _params, CultureInfo.CurrentCulture);
			if (_method is object)
		...
		public Traverse Type(string name)
		{
			...
			var type = AccessUtils.Inner(_type, name);
		...
		{
			var resolved = Resolve();
			return AccessUtils.GetFieldNames(resolved._type);
		}
		...
		{
			var resolved = Resolve();
			return AccessUtils.GetPropertyNames(resolved._type);
		}
		...
		public Traverse Method(string name, params object[] arguments)
		{
			...
			var resolved = Resolve();
			if (resolved._type is null) return new Traverse();
			var types = AccessUtils.GetTypes(arguments);
		...
		{
			var resolved = Resolve();
			return AccessUtils.GetMethodNames(resolved._type);
		}
		...
		public static void IterateFields(object source, Action<Traverse> action)
		{
			...
			AccessUtils.GetFieldNames(source).ForEach(f => action(sourceTrv.Field(f)));
		}
		...
		public static void IterateFields(object source, object target, Action<Traverse, Traverse> action)
		{
			...
			AccessUtils.GetFieldNames(source).ForEach(f => action(sourceTrv.Field(f), targetTrv.Field(f)));
		}
		...
		public static void IterateFields(object source, object target, Action<string, Traverse, Traverse> action)
		{
			var sourceTrv = Create(source);
			var targetTrv = Create(target);
			AccessUtils.GetFieldNames(source).ForEach(f => action(f, sourceTrv.Field(f), targetTrv.Field(f)));
		}
		...
		{
			var sourceTrv = Create(source);
			AccessUtils.GetPropertyNames(source).ForEach(f => action(sourceTrv.Property(f)));
		}
		...
		public static void IterateProperties(object source, object target, Action<Traverse, Traverse> action)
		{
			var sourceTrv = Create(source);
			var targetTrv = Create(target);
			AccessUtils.GetPropertyNames(source).ForEach(f => action(sourceTrv.Property(f), targetTrv.Property(f)));
		}
		...
		public static void IterateProperties(object source, object target, Action<string, Traverse, Traverse> action)
		{
			var sourceTrv = Create(source);
			var targetTrv = Create(target);
			AccessUtils.GetPropertyNames(source).ForEach(f => action(f, sourceTrv.Property(f), targetTrv.Property(f)));
		}
