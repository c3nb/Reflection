...
namespace System.Reflection.Utils
{
	...
	public class PatchInfo
	{
		...
		/// <param name="owner">An owner (Runtime ID)</param>
		/// <param name="methods">The patch methods</param>
		///
		internal void AddPrefixes(string owner, params RuntimeMethod[] methods)
		{
		...
		{
			AddPrefixes(owner, new RuntimeMethod(patch, priority, before, after, debug));
		}
		...
		/// <param name="methods">The patch methods</param>
		///
		internal void AddPostfixes(string owner, params RuntimeMethod[] methods)
		{
		...
		{
			AddPostfixes(owner, new RuntimeMethod(patch, priority, before, after, debug));
		}
		...
		/// <param name="methods">The patch methods</param>
		///
		internal void AddTranspilers(string owner, params RuntimeMethod[] methods)
		{
		...
		{
			AddTranspilers(owner, new RuntimeMethod(patch, priority, before, after, debug));
		}
		...
		/// <param name="methods">The patch methods</param>
		///
		internal void AddFinalizers(string owner, params RuntimeMethod[] methods)
		{
		...
		{
			AddFinalizers(owner, new RuntimeMethod(patch, priority, before, after, debug));
		}
		...
		/// <param name="owner">The Runtime instance ID adding the new patches</param>
		/// <param name="add">The patches to add</param>
		/// <param name="current">The current patches</param>
		///
		private static Patch[] Add(string owner, RuntimeMethod[] add, Patch[] current)
		{
	...
	public class Patch : IComparable
	{
		...
		/// <summary>The owner (Runtime ID)</summary>
		///
		...
		/// <summary>Keep this patch before the patches indicated in the list of Runtime IDs</summary>
		///
		public readonly string[] before;

		/// <summary>Keep this patch after the patches indicated in the list of Runtime IDs</summary>
		///
		...
		/// <param name="before">A list of Runtime IDs for patches that should run after this patch</param>
		/// <param name="after">A list of Runtime IDs for patches that should run before this patch</param>
		...
		///
		public Patch(MethodInfo patch, int index, string owner, int priority, string[] before, string[] after, bool ignoreDynamicMethod)
		{
			if (patch is DynamicMethod && !ignoreDynamicMethod) throw new Exception($"Cannot directly reference dynamic method \"{patch.FullDescription()}\" in Runtime. Use a factory method instead that will return the dynamic method.");
		...
		/// <param name="index">Zero-based index</param>
		/// <param name="owner">An owner (Runtime ID)</param>
		public Patch(RuntimeMethod method, int index, string owner, bool ignoreDynamicMethod = false)
			: this(method.method, index, owner, method.priority, method.before, method.after, ignoreDynamicMethod) { }
